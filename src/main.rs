...
/// clear && cargo build && ./target/debug/two_phase_commit -s .95 -c 4 -p 10 -r 10 -m run
/// clear && cargo run -s .95 -c 4 -p 10 -r 10 -m check -v 0
/// 
/// 2PC: Phase 1
/// 1. Coordiantor send REQUEST to all participants
///     responsible for final transitions
/// 2. Particants recieve request and                           (if crash, doesn't send vote, sees timeout and coord abords)
/// 3. Execute locally, don't execute state
/// 4. Write VOTE_COMMIT or VOTE_ABORT to local log
/// 5. Send VOTE_COMMIT or VOTE_ABORT to coordinator
/// 
/// 2PC: Phase 2 (Perspective of Coordinator)
/// Case 1: recieve VOTE_ABORT or timeout (gets nothing back)   (if crash, no write, no commit, trans doesn't take place)
///     Handles by GLOBAL_ABORT to log
///     send GLOBAL_ABORT to participants
/// Case 2: recieve VOTE_COMMIT from all
///     Write GLOBAL_COMMIT to log (step that trans to not-comm to comm)    (if crash, before write, equ to timeout, if commit and crash, when comes up look at log)
///     send GLOBAL_COMMIT to partcipants           
/// Participants receive decision, write                        (if crash, ask coord, what happen, look at log, and act)
///     GLOBAL_ABORT or GLOBAL COMMIT to own logs and go on
/// 
